...
async def open_tab_and_scrape(browser, page_range):
    page = await browser.new_page()
    await login_kalodata(page)
    await shop_main(page, page_range)
...
async def run():
    async with async_playwright() as p:
        ...
        for page_range in tasks:
            asyncio.create_task(open_tab_and_scrape(browser, page_range))
            delay = random.randint(120, 180)  # 5 to 7 minutes in seconds
